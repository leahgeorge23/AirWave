...
def run_voice_and_fft(loop: asyncio.AbstractEventLoop):
    ...
    mic_index = getattr(vc, "DEVICE_INDEX", 0)
    ...
    # Minimum speech constraints to reduce false triggers
    MIN_SPEECH_S = 0.12
    MIN_PEAK_RMS = 750.0
            ...
            if rms > 900:  # tune if needed
                ...
                if speech_active:
                    ...
                    if now - last_voice_time > 0.5:
                        ...
                        speech_duration = len(raw) / 2 / RATE
                        if speech_duration < MIN_SPEECH_S or speech_peak_rms < MIN_PEAK_RMS:
                            print(f"[VOICE] Ignored short/quiet speech (dur={speech_duration:.2f}s peak={speech_peak_rms:.0f})")
                        ...
                        try:
                            ...
                            if hasattr(vc, "recognize_offline_with_partial"):
                                text, partial = vc.recognize_offline_with_partial(audio)
                            else:
                                text, partial = vc.recognize_offline(audio), None
                            if not text:
                                if partial:
...
def run_voice_detection(loop: asyncio.AbstractEventLoop):
    ...
    try:
        sample_rate = getattr(vc, "SAMPLE_RATE", 16000)
        chunk_size = getattr(vc, "CHUNK", 1024)
        with sr.Microphone(
            device_index=mic_index,
            sample_rate=sample_rate,
            chunk_size=chunk_size,
        ) as source:
            print(f"[VOICE] Microphone opened (device={mic_index}, rate={sample_rate}, chunk={chunk_size})")
            ...
            while True:
                ...
                try:
                    ...
                    try:
                        ...
                        speech_duration = len(raw) / 2 / sample_rate
                        if speech_duration < 0.12:
                            print(f"[VOICE] Ignored short speech (dur={speech_duration:.2f}s)")
                            print("[VOICE] Ready for next command")
                            continue
                        if hasattr(vc, "recognize_offline_with_partial"):
                            text, partial = vc.recognize_offline_with_partial(audio)
                        ...
                        print(f"[VOICE] Offline STT error: {e}")
...
async def main():
    ...
    print("[VOICE] Offline STT enabled (vosk) - no Google calls")
    print("=" * 60)
    ...
    try:
        while True:
            if not gesture_enabled:
                # Keep loop alive but skip BLE/IMU when gestures are disabled.
