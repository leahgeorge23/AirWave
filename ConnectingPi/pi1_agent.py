...
LIGHT_DANCE_PULSE_SOURCE = os.environ.get(
    "LIGHT_DANCE_PULSE_SOURCE",
    "alsa_input.usb-C-Media_Electronics_Inc._USB_PnP_Sound_Device-00.analog-mono.2"
).strip()

LIGHT_DANCE_ENABLED = True
_light_dance_pause_until = 0.0
_light_dance_thread_started = False

def _pause_light_dance(seconds: float):
    global _light_dance_pause_until
    t = time.time() + max(0.0, float(seconds))
    if t > _light_dance_pause_until:
        _light_dance_pause_until = t

def _light_dance_is_paused() -> bool:
    return time.time() < _light_dance_pause_until

def start_light_dancing():
    global _light_dance_thread_started
    if _light_dance_thread_started:
        return
    _light_dance_thread_started = True
    t = threading.Thread(target=_light_dance_loop, daemon=True)
    t.start()

def _light_dance_loop():
    if not LED_AVAILABLE:
        print("[LED] Light dancing disabled (LED not available / not sudo / neopixel init failed).")
        return

    if not LIGHT_DANCE_PULSE_SOURCE:
        print("[LED] LIGHT_DANCE_PULSE_SOURCE is empty; light dancing disabled.")
        return
    ...
    cmd = [
        "parec",
        "--device", LIGHT_DANCE_PULSE_SOURCE,
        "--format=s16le",
        f"--rate={rate}",
        "--channels=1",
    ]
    ...
    try:
        ...
        print(f"[LED] Failed to start parec: {e}")
        return

    print(f"[LED] Light dancing active from: {LIGHT_DANCE_PULSE_SOURCE}")
    ...
    try:
        while True:
            if not LIGHT_DANCE_ENABLED or _light_dance_is_paused():
                time.sleep(0.01)
                continue
...
# Higher gain + non-linear mapping so quiet audio still spreads across more LEDs
            ...
            if n_lit < 1 and level > 0.02:
                n_lit = 1
            phase += 0.35 + 2.2 * level
            r = int(255 * level)
            g = int(120 * (0.5 + 0.5 * math.sin(phase)))
            b = int(255 * (0.5 + 0.5 * math.cos(phase * 0.7)))
