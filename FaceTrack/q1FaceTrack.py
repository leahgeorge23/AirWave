...
def print_available_trackers():
    """Debug: print what trackers are available."""
...
def create_tracker():
    ...
    # Try different APIs based on OpenCV version
...
def detect_person(frame, face_cascade, profile_cascade, upper_body_cascade):
    """Try to detect a person using multiple cascades."""
    ...
    if not profile_cascade.empty():
        ...
        # Try flipped frame for other profile direction
...
def lock_onto_person(cap, face_cascade, profile_cascade, upper_body_cascade):
    """Wait for a person and initialize tracker."""
    ...
    while True:
        ...
        if bbox is not None:
            ...
            # Initialize tracker - bbox must be tuple of (x, y, w, h)
            try:
...
def main():
    ...
    lost_frames = 0
    MAX_LOST_FRAMES = 45
    detect_interval = 3  # For fallback mode
    frame_count = 0
    ...
    try:
        while True:
            ...
            if use_tracker:
                ...
                # Fallback: run detection periodically
                frame_count += 1
            ...
            if success and bbox is not None:
                ...
                # Distance estimation from area
                size_ratio = ref_area / float(area) if area > 0 else 1.0
                distance_ft = REF_DISTANCE_FEET * np.sqrt(size_ratio)
