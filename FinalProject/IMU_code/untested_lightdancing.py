...
def get_default_monitor_source() -> str:
    # PulseAudio exposes a "monitor" source for each sink. We read from the default sink's monitor.
    default_sink = run("pactl get-default-sink")
    sources = run("pactl list short sources").splitlines()
    ...
    if monitor is None:
        # fallback: try any monitor source
        for line in sources:
            parts = line.split()
    ...
    if monitor is None:
        raise RuntimeError("No PulseAudio monitor source found. Is PulseAudio running and is there an audio sink?")

    return monitor

def open_parec(monitor: str):
    # parec outputs raw PCM. We'll read bytes in Python.
    cmd = (
        f"parec --device={monitor} "
        f"--format=s16le --rate={RATE} --channels={CHANNELS}"
    )
    return subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, bufsize=0)
...
def compute_features(samples: np.ndarray, prev_energy_smooth: float, prev_beat_level: float):
    ...
    # Smooth energy for stable brightness
    ...
    # band splits (tweak if you want)
    low_e = band_energy(20, 200)
    mid_e = band_energy(200, 2000)
    high_e = band_energy(2000, 8000)

    # Normalize bands by total to get color proportions
    total = low_e + mid_e + high_e + 1e-9
    low = low_e / total
    mid = mid_e / total
    high = high_e / total

    # Normalize energy to something usable
    # These constants depend on your volume; adjust if too dim/too bright.
    energy = clamp01((energy_smooth - 0.01) / 0.08)

    # Simple beat detection using low band “punch”
...
def main():
    print("Bluetooth Spotify light-dancing (PulseAudio monitor). Ctrl+C to stop.")
    monitor = get_default_monitor_source()
